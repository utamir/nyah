...
var fll = (level, arguments) => [fcd(new Date()), Object.keys(log.levels).filter(key=>log.levels[key] === level), ((new Error().stack).split("at ")[3]).trim().replace(/([A-Z]:)?[\/\\].*[\/\\]/,''), util.format.apply(util,arguments)].join(' | ');
log.debug = function(){
