...
function piwlan(iface) {
 if (!(this instanceof piwlan))
  return new piwlan(iface);
 this.iface = iface;
}
  ...
  exec(wcon, function (e, r) {
   if (e) {
    ...
    exec(ex, function (e, r) {
     if (e || r == 'FAIL')
      err('Unable to connect to ' + net.ssid);
     else {
...
piwlan.prototype.disconnect = function (net, remove) {
 ...
 return new Promise(function (res, err) {
  ...
  exec(wcon, function (e, r) {
   if (e || r == 'FAIL')
    err('Unable to disconnect from ' + net.ssid);
   else
    res(net.ssid + ' is disconnected' + (remove ? ' and removed' : ''));
  });
 });
}